  ...
  return (
    <div className="min-h-screen bg-[#FFF9F0] py-8 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {!isEditMode && childrenProfiles.length > 0 && (
          <div className="mb-6">
            <h2 className="text-xl font-black text-[#4A4A4A] mb-4 flex items-center gap-2">
              <span>📊</span> Laporan Anak
            </h2>
            <div className="grid gap-4">
              {childrenProfiles.map((child) => (
                <Card key={child.id} className="bg-white p-4 rounded-xl border-2 border-[#FFE0B2] flex items-center justify-between hover:border-[#FF7A00] transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-[#FFF5E5] rounded-full flex items-center justify-center text-xl font-bold text-[#FF7A00] border-2 border-[#FFE0B2]">
                      {child.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <h3 className="font-bold text-[#4A4A4A]">{child.name}</h3>
                      <p className="text-sm text-[#8B7355]">{child.age_group} Tahun</p>
                    </div>
                  </div>
                  <Link href={`/report?name=${child.name}&id=${child.id}`}>
                    <Button variant="ghost" className="text-[#FF7A00] hover:bg-[#FFF5E5] font-bold">
                      Lihat Laporan
                    </Button>
                  </Link>
                </Card>
