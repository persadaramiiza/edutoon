...
function WatchPage() {
  ...
  // ==================== HANDLE BACK ====================
  const handleBack = () => {
    // Kembali ke dashboard (layar menonton)
    if (profileId > 0) {
  ...
  const loadVideo = useCallback(async () => {
    try {
      ...
      // Increment view and update local state
      videosService.incrementView(videoId).then(() => {
        setVideo(prev => prev ? { ...prev, view_count: (prev.view_count || 0) + 1 } : null);
      }).catch(err => {
  ...
  // ==================== PROGRESS SAVER LOGIC ====================
  const startProgressSaver = useCallback(() => {
    if (progressIntervalRef.current) return;
    
    console.log('💾 Starting progress saver');
  ...
  const stopProgressSaver = useCallback(() => {
    if (progressIntervalRef.current) {
      console.log('🛑 Stopping progress saver');
  ...
  }, [isMounted, video, videoProgress, getYouTubeVideoId, startQuizCheck, stopQuizCheck, handleVideoEnd, stopProgressSaver]);
