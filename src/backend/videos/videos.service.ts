...
export class VideosService {
  ...
  // ==================== GET RECOMMENDATIONS ====================
  async getRecommendations(ageGroup: number): Promise<Video[]> {
    console.log(`🎯 Getting recommendations for age group: ${ageGroup}`);
    
    return this.videoRepo.find({
      where: {
        status: 'published',
        min_age: LessThanOrEqual(ageGroup),
        max_age: MoreThanOrEqual(ageGroup),
      },
      order: {
        view_count: 'DESC',
        created_at: 'DESC',
