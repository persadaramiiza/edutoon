...
import { PlayCircle, ShieldCheck, Brain, Users, Star, Heart, CheckCircle2, Menu, X } from "lucide-react"
    ...
    <div className="flex min-h-screen flex-col bg-[#FFF9F0] font-sans overflow-x-hidden text-[#4A4A4A]">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b-2 border-[#FFE0B2] shadow-sm h-20 md:h-24 transition-all duration-300">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-20 left-0 w-full bg-white border-b border-[#FFE0B2] p-6 flex flex-col gap-4 shadow-xl animate-in slide-in-from-top-5">
             {["Features", "For Parents", "Creators"].map((item) => (
                <Link key={item} href="#" className="text-[#8B7355] font-bold text-lg">{item}</Link>
             ))}
             <div className="flex flex-col gap-3 mt-4">
                ...
                <Link href="/register">
                  <Button className="w-full bg-[#FF7A00] font-bold text-lg text-white hover:bg-[#E66E00]">Sign Up Now</Button>
                </Link>
             </div>
          </div>
        )}
      ...
      <main className="flex-1"> 
        ...
        <section className="relative flex flex-col justify-start w-full">
          ...
          {/* Menggunakan w-full h-auto agar gambar menyesuaikan lebar layar dan tinggi mengikuti aspek rasio (tidak terpotong) */}
          <div className="relative w-full z-0 mt-20 md:mt-24"> 
             ...
             {/* Gradasi halus di bawah gambar untuk menyambungkan dengan background krem */}
             {/* UPDATE: Tinggi gradasi dinaikkan (h-32 md:h-64) agar lebih smooth naik ke atas */}
          ...
          {/* Konten ini secara alami akan berada di bawah gambar karena div gambar di atas memiliki h-auto */}
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 mb-12 md:mb-16">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center px-4">
                  ...
                  <Link href="/parent/dashboard" className="w-full sm:w-auto">
                    <Button 
                        size="lg" 
              ...
              <div className="relative w-full max-w-5xl mx-auto px-4">
                  <div className="relative aspect-video rounded-[1.5rem] md:rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.2)] border-[6px] md:border-[8px] lg:border-[12px] border-[#FFE0B2] bg-gray-900 group z-20">
                    <img 
                    ...
                    <div className="absolute inset-0 flex items-center justify-center z-10">
                      <div className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border-4 border-white/50 shadow-[0_10px_30px_rgba(248,94,0,0.4)] animate-pulse cursor-pointer hover:scale-110 transition-transform group-hover:bg-[#FF7A00]/80">
                        <PlayCircle className="h-8 w-8 md:h-10 md:w-10 lg:h-12 lg:w-12 text-white ml-1" />
                      </div>
                    </div>

                    <div className="absolute top-3 right-3 md:top-6 md:right-6 lg:top-8 lg:right-8 bg-white px-3 py-1.5 md:px-4 md:py-2 rounded-full shadow-lg flex items-center gap-1.5 md:gap-2 animate-bounce delay-1000 z-20">
                      <Star className="h-4 w-4 md:h-5 md:w-5 text-[#FFD93D] fill-[#FFD93D]" />
                      <span className="font-black text-[#8B7355] text-xs md:text-sm lg:text-base">500 Points</span>
                    </div>

                    <div className="absolute bottom-3 left-3 right-3 md:bottom-6 md:left-6 md:right-6 lg:bottom-8 lg:left-1/2 lg:-translate-x-1/2 lg:w-full lg:max-w-md z-30">
                      <div className="bg-white p-3 md:p-4 lg:p-6 rounded-[1rem] md:rounded-[1.5rem] lg:rounded-[2rem] shadow-2xl border-b-4 md:border-b-6 lg:border-b-8 border-[#FF7A00] animate-in slide-in-from-bottom-4 fade-in duration-700 delay-500">
                        <div className="flex gap-2 md:gap-3 lg:gap-4 items-center">
                          ...
                          <div className="flex-1 text-left">
                            <h3 className="font-black text-[#8B7355] mb-0.5 md:mb-1 text-sm md:text-base lg:text-lg flex items-center gap-2">
                              Quick Quiz! 
                              {quizAnswered === true && <CheckCircle2 className="h-4 w-4 md:h-5 md:w-5 text-green-500" />}
                            </h3>
                            <p className="text-xs md:text-sm text-[#8B7355]/70 font-bold mb-2 md:mb-3 leading-tight">
                            ...
                            {quizAnswered === null ? (
                              <div className="flex gap-2">
                                <button 
                                  ...
                                  className="px-2 py-1.5 md:px-3 rounded-lg bg-gray-100 text-[#8B7355] text-xs md:text-sm font-bold border-b-2 border-gray-200 hover:bg-gray-200 hover:scale-105 transition-all w-full text-center"
                                >
                                  Elephant
                                </button>
                                <button 
                                  onClick={() => handleQuizAnswer(true)}
                                  className="px-2 py-1.5 md:px-3 rounded-lg bg-[#FF7A00] text-white text-xs md:text-sm font-bold border-b-2 border-[#E66E00] hover:bg-[#FF7A00]/90 hover:scale-105 transition-all shadow-lg w-full text-center"
                                >
                                  Giraffe
                                </button>
                              </div>
                            ) : (
                              <div className={`text-xs md:text-sm font-bold p-2 rounded-lg text-center ${quizAnswered ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                {quizAnswered ? "Correct! 🎉 +50 Points" : "Try Again! 😅"}
                                <button onClick={() => setQuizAnswered(null)} className="block w-full text-xs mt-1 underline opacity-70 hover:opacity-100">
                                    {quizAnswered ? "Play Again" : "Try Again"}
                                </button>
                              </div>
                            )}
                          </div>
                  ...
                  <div className="absolute -top-8 -right-8 md:-top-16 md:-right-16 lg:-top-24 lg:-right-24 w-32 h-32 md:w-48 md:h-48 lg:w-64 lg:h-64 pointer-events-none z-10 hidden md:block">
                     <img src="/images/new-dragon.png" alt="Dragon" className="w-full h-full object-contain drop-shadow-2xl transform rotate-12" />
                  </div>
              </div>
            </div>
          </div>
            ...
            <div className="relative z-10 flex-1 text-center md:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                <Link href="/register" className="w-full sm:w-auto">
                  <Button size="lg" className="bg-[#FFD93D] text-[#4A4A4A] hover:bg-white border-b-4 border-[#FF7A00] w-full text-lg md:text-xl h-14 md:h-16 px-8 md:px-10 rounded-2xl font-black">
                    Get Started Free
                  </Button>
                </Link>
              </div>
      ...
      <style jsx global>{`
        ...
        @keyframes bounce {
          0%, 100% { transform: translateY(-5%) rotate(6deg); animation-timing-function: cubic-bezier(0.8,0,1,1); }
          50% { transform: translateY(0) rotate(6deg); animation-timing-function: cubic-bezier(0,0,0.2,1); }
        }
      `}</style>
    </div>
  )
